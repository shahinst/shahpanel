...
@section('page_actions')
    @can('create', \App\Models\Account::class)
        <x-button :href="route('agent.accounts.create')" size="sm">
...
<x-page-header :title="__('menu.accounts')">
    <x-slot:actions>
        @can('create', \App\Models\Account::class)
            <x-button :href="route('agent.accounts.create')">
                <i class="bx bx-plus align-middle"></i> {{ __('accounts.create') }}
            </x-button>
        @endcan
    </x-slot:actions>
                ...
                <x-table :headers="[__('ui.col_user'), __('accounts.package'), __('app.status'), __('accounts.expiry'), __('app.actions')]">
                    @forelse ($accounts as $account)
                        <tr>
                            ...
                            <td>@include('shared.accounts.status-toggle', ['account' => $account, 'prefix' => 'agent'])</td>
                            <td>@include('shared.accounts.expiry-cell', ['account' => $account])</td>
                            <td>@include('shared.accounts.wireguard-menu', ['account' => $account, 'prefix' => 'agent'])</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted py-4">
                                {{ __('app.no_results') }}
                                @can('create', \App\Models\Account::class)
                                    — <a href="{{ route('agent.accounts.create') }}">{{ __('accounts.create') }}</a>
                                @endcan
                            </td>
                        </tr>
                    @endforelse
